...
test("Decidir el siguiente salto teniendo dos(2) filas",()=>{
    ...
    expect(mapa.siguienteSalto(paquete1,[0,0])).toEqual([1]);
}); 
...
test("Que el paquete con mayor urgencia llegue a destino primero",()=>{
    ...
    let ttl3=paquete3.ttl -5;
        ...
        mapa.pasarTurno(0);
        expect(distribucion1.colaDeEspera[0].id).toBe(id1);
        expect(distribucion2.colaDeEspera[0].id).toBe(id2);
...
test("Ofrecer todas las movidas posibles para un paquete",()=>{
    ...
    expect(mapa.siguienteSalto(paquete1,[1,0])).toEqual([0,0,1,2]);
